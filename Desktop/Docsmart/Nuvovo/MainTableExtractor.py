...
class MainTableExtractor:
    ...
    def execute(self):
        ...
        # Apply Gaussian blur to reduce noise
        self.blurred = cv2.GaussianBlur(self.grey, (5, 5), 5)

        # Apply adaptive thresholding
        self.thresholded_image = cv2.adaptiveThreshold(
            self.blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 6
        )

        cv2.imshow("thresh",self.thresholded_image)
        cv2.waitKey(0)
