...
class OcrToTableTool:
    ...
    def dilate_image(self):
        kernel = np.ones((2, 5), np.uint8)
        self.dilated_image = cv2.dilate(self.thresholded_image, kernel, iterations=6)

    def find_contours(self):
        self.image_with_contours_drawn = self.original_image.copy()
        contours, _ = cv2.findContours(self.dilated_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        self.contours = [contour for contour in contours if cv2.contourArea(contour) > 800 ]
        cv2.drawContours(self.image_with_contours_drawn, self.contours, -1, (0, 255, 0), 2)
    ...
    def print_table_to_console(self):
        ...
        non_empty_rows = [row for row in self.table if any(cell.strip() for cell in row)]

        for index, row in enumerate(non_empty_rows):
            row_data = ", ".join(row)
            print(f"Row {index + 1}: {row_data}")

            # Check if the row starts with "Si.No" or ends with "Amount"
            if row and (row[index].lower().startswith("si.no") or 
            row[index].lower().startswith("sr") or 
            row[index].lower().startswith("sr.no") or 
            row[index].lower().startswith("si") or
            (row[-1].lower() == "amount" or row[-1].lower() == "amount in ₹")):

                header_array = row  # Current row as header array
                print("\nHeader Array Identified:")
                print(header_array)

                # Fetch the next row if available
                if index + 1 < len(non_empty_rows):
                    value_array = non_empty_rows[index + 1]
                    print("\nValue Array Identified:")
                    print(value_array)
                break  # Stop after finding the first matching row and its values

        if not header_array:
            print("\nNo matching header row found.")
        if not value_array:
            print("\nNo matching value row found.")
